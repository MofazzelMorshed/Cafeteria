...
public class Simulation {
    ...
    public static Map<Customer, Boolean> Order = new ConcurrentHashMap<Customer, Boolean>();
    ...
    public static List<SimulationEvent> runSimulation(
        ...
        // Now "let the customers know the shop is open" by starting them running in their own thread.
        for (int i = 0; i < customers.length; i++) {
            customers[i].start();
            /*NOTE: Starting the customer does NOT mean they get to go right into the shop.  There has to be a table for them.  
            The Customer class' run method has many jobs to do - one of these is waiting for an available table...*/
        }
        ...
        try {
            ...
            /*Then send cooks home..The easiest way to do this might be the following,where we interrupt 
             their threads.  There are other approaches   though, so you can change this if you want to.*/
            for (int i = 0; i < cooksCooking.length; i++) {
