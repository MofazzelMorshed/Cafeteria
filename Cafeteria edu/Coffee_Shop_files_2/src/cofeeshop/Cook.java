...
public class Cook implements Runnable {
    ...
    public List<Food> foodCompletedList = Collections.synchronizedList(new LinkedList<>());
        ...
        try {
            while (true) {
                synchronized (Simulation.listForOrders) {
                    ...
                    Simulation.logEvent(
                            SimulationEvent.cookReceivedOrder(this, customer.getOrder(), customer.getOrderNum()));
                }
                for (int i = 0; i < customer.getOrder().size(); i++) {
                    ...
                    if (currentFoodOrder.equals(FoodType.burger)) {
                        ...
                        Simulation.logEvent(
                                SimulationEvent.cookStartedFood(this, FoodType.burger, customer.getOrderNum()));
                        ...
                        Simulation.logEvent(
                                SimulationEvent.cookStartedFood(this, FoodType.fries, customer.getOrderNum()));
                        ...
                        Simulation.logEvent(
                                SimulationEvent.cookStartedFood(this, FoodType.coffee, customer.getOrderNum()));
                    }
                }
                // synchronized(foodCompletedList){
                while (customer.getOrder().size() != foodCompletedList.size()) {
                    // System.out.println("Wait till the food list to get completed");
                }
                Simulation.logEvent(SimulationEvent.cookCompletedOrder(this, customer.getOrderNum()));
                foodCompletedList = Collections.synchronizedList(new LinkedList<>());
